## Simulationslauf vom: 2025-11-10 20:52:41

### Globale Parameter
- **Begegnungsschema:** `SpatialGridScheme`
- **Gittergröße:** `(10, 20)`
- **Anzahl Matches/Duelle:** `60000`
- **Runden pro Episode:** `200`
- **Zufalls-Seed:** `6`

### Agenten-Population
- **QL:** `200`
- **Gesamt:** `200` Agenten

### Lern-Hyperparameter
- **alpha:** `0.05`
- **gamma:** `0.95`
- **epsilon:** `1.0`
- **epsilon_decay:** `0.9995`
- **min_epsilon:** `0.01`

---

### Finale Ergebnisse (des obigen Laufs)

**Globale Metriken (Gesamtsystem):**
- **System-Effizienz (% des Max):** `46.32%`
- **Durchschnittliche Kooperationsrate:** `15.45%`

**Metriken pro Agententyp (Durchschnitt über den gesamten Zeitraum):**
- **Avg. Reward/Match:**
  - `QLearningAgent`: 277.72
- **Avg. Kooperationsrate:**
  - `QLearningAgent`: 15.45%

**Finale Cluster-Analyse:**
- **Anzahl gefundener Cluster:** `2`
- **Gesamtfläche pro Strategietyp:**
  - `Defector`: 75.5%
  - `Polarized`: 8.0%
  - `Cautious Coop.`: 6.5%
  - `TFT-like`: 5.0%
  - `Cooperator (ALLC)`: 3.0%
  - `WSLS`: 2.0%
- **Anzahl Cluster pro Strategietyp:**
  - `Defector`: 2

---

